...
def direction_loss(p, q, directions, n=100, in_dim=3, device='cuda:0'):
    ...
    for i in range(len(p_slices)):
        projections_q = (q_slices[i].double() @ directions).unsqueeze(dim=0)
        projections_p = (p_slices[i].double() @ directions).unsqueeze(dim=0)
    ...
    return torch.mean(torch.stack(losses)) #- centroid_distance(q.data, p.data)
